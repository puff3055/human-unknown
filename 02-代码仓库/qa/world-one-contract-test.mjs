...
const worldStyle = await readFile(join(root, 'world-one.css'), 'utf8');
...
assert.match(world, /handlePointerMove/, 'desktop pointer interaction must exist');
assert.match(world, /handleKeyDown/, 'keyboard interaction must exist');
assert.match(world, /startGuidedAction/, 'each causal scene must expose a visible guided action');
assert.match(world, /updateInteractionUi/, 'interaction progress must remain visible');
assert.match(index, /id="worldInteractionMeter"/, 'the desktop action bar must expose progress');
assert.match(world, /transitionMode/, 'scene changes must declare a spatial transition mode');
assert.match(worldStyle, /--world-stage-ratio/, 'the desktop stage must preserve the source aspect ratio');
assert.match(worldStyle, /1672px/, 'the desktop stage must not upscale beyond the source width');
assert.match(worldStyle, /941px/, 'the desktop stage must not upscale beyond the source height');
assert.doesNotMatch(worldStyle, /object-fit:\s*cover/, 'source masters must never be cropped with cover');
assert.doesNotMatch(worldStyle, /world-one__scene-image[^}]*transform:/s, 'settled source masters must not be softened by transforms');
assert.doesNotMatch(worldStyle, /is-leaving/, 'the scene system must not fall back to slideshow dissolves');
...
console.log('World one desktop contract passed: six ordered source-faithful scenes, native-size clarity, visible causality, continuity, keyboard fallback, honest ending, and independent scene layers.');
