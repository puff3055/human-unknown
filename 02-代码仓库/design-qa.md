...
> 日期：2026-09-06
> 对象：`codex/light-cursor-v4-3`／`v4.3.0-rc.2`
...
- **[P1] 用户在实际入口仍看到系统箭头。**核对发现 `4174` 服务的是另一份 V3.4／植物世界工作目录，GitHub Pages 服务的是尚未包含光标的 V4.2，而本分支临时 `4182` 服务已停止；三者不是同一入口。
- **修正：**重新启动本分支预览，并把精细指针环境的 `cursor: none` 扩展到整页全部后代和伪元素，防止按钮、画布或宿主默认样式恢复系统箭头；粗指针媒体查询仍显式恢复原生光标。
