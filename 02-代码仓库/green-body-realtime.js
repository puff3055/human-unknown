(() => {
  ...
  function setPhase(phase) {
    ...
    if (phase === 'falling') showBeat('falling');
    ...
    if (phase === 'whole') hint.firstChild.nodeValue = '没有一处，独自活着。 ';
  ...
  function ignite(node, originPoint = null) {
  ...
  function update(delta, now) {
    ...
    if (state.active) {
      if (elapsed < 3.5 && state.phase !== 'network' && state.phase !== 'whole') setPhase('falling');
      else if (elapsed < 7.2 && state.phase !== 'network' && state.phase !== 'whole') setPhase('chapter');
      else if (state.phase === 'falling' || state.phase === 'chapter') setPhase('local');
    }
  ...
  function render(now) {
    ...
    if (state.active) {
      drawNetwork(now);
      if (state.phase === 'falling') drawFalling(now);
    }
  ...
  function enter(options = {}) {
    ...
    state.entering = true;
    const delay = options.immediate || reduceMotion ? 30 : 520;
    document.getElementById('contact').classList.add('is-entering');
    setTimeout(() => {
      root.hidden = false;
  ...
  function exit() {
    ...
    const contact = document.getElementById('contact');
    contact.inert = false;
    contact.classList.remove('is-world-hidden', 'is-entering');
    contact.dataset.entryHolding = 'false';
    contact.style.setProperty('--entry-progress', '0');
    window.__humanUnknown?.resume?.();
    showBeat('');
    setTimeout(() => { if (!state.active) root.hidden = true; }, 450);
  }

  function updateEntrance() {
    requestAnimationFrame(updateEntrance);
    if (state.active || state.entering || !window.__humanUnknown?.getState) return;
    const home = window.__humanUnknown.getState();
    const gaze = home.gaze || { x: 0, y: 0 };
    const distance = Math.hypot(state.pointer.x - (innerWidth / 2 + gaze.x), state.pointer.y - (innerHeight / 2 + gaze.y));
    const inside = home.phase === 'closer' && state.pointer.visible && distance < Math.max(42, Math.min(innerWidth, innerHeight) * .08);
    state.entrance = clamp(state.entrance + (inside ? 1 / 95 : -1 / 38), 0, 1);
    document.getElementById('contact').dataset.entryHolding = String(inside);
    document.getElementById('contact').style.setProperty('--entry-progress', state.entrance.toFixed(3));
    if (state.entrance >= 1) enter();
  }
  ...
  addEventListener('pointermove', (event) => {
    ...
    cursor.classList.toggle('is-visible', state.active);
    if (!state.active || (state.phase !== 'local' && state.phase !== 'network' && state.phase !== 'whole')) return;
  ...
  addEventListener('pointerdown', (event) => {
    ...
    ignite(node, { x: event.clientX, y: event.clientY });
  }, { passive: true });
  addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && state.active) exit();
    if ((event.key === 'Enter' || event.key === ' ') && !state.active && window.__humanUnknown?.getState?.().phase !== 'waiting') enter();
    if ((event.key === 'Enter' || event.key === ' ') && state.active) ignite(nearestNode(state.width / 2, state.height / 2, false), { x: state.width / 2, y: state.height / 2 });
  });
