(() => {
  ...
  function handleWheel(event) {
    ...
    state.targetDepth = clamp(state.targetDepth + delta * .00135, -1.8, 5.8);
  ...
  function update(now, delta) {
    ...
    if (state.clickedAt >= 0) {
      ...
      if (now >= state.diveAt) {
        ...
        if (diveProgress >= 1) {
          ...
          prompt.textContent = "滚动继续改变尺度。点击，再注入一次信号。";
      ...
      if (Math.abs(depthDelta) < .0008 && state.phase === "exploring") setPhase("free");
    }
    ...
    sound.setPointerEnergy(pointer.energy); scheduleAmbientEvent(now);
  }
  ...
  async function initialize() {
    try {
      ...
      status.textContent = "滚动改变观察距离；点击任意球形节点注入信号。";
